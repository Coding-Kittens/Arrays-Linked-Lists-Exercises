...
describe("average", function() {
  ...
  it("ignores non number items", function() {
    let lst = new LinkedList([2, 3, 1,'cat', 1, 7,'cat', 6, 9,'cat']);
    expect(lst.average()).toBeCloseTo(4.1429, 4);
  });

  it("returns 0 if there are only non number items", function() {
    let lst = new LinkedList(['cat','cat','cat']);
    expect(lst.average()).toBe(0);
  });
